...
from datetime import datetime, timedelta  # 導入 timedelta 修正時區
import urllib.parse
        ...
        try:
            with st.spinner('分析中...'):
                ...
                def get_stats(df, pos_keywords, sim_keywords):
                    ...
                    # 關鍵修正：只抓取「提單號碼」不為空的行，排除掉像第 3 行那樣的空白行
                    valid_rows = temp_df[temp_df['提單號碼'].astype(str).str.strip() != '']
                    
                    for _, r in valid_rows.iterrows():
                        a = str(r['報關'])
                        p = str(r['寄件人']).strip()
                        short_p = p.replace("有限公司", "").replace("股份有限公司", "")
